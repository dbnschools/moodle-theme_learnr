@theme @theme_learnr @theme_learnr_flavourssettings @theme_learnr_flavourssettings_look
Feature: Configuring the theme_learnr plugin on the "Flavours" page, applying the look features
  ...
  Scenario: Flavours: Compact logo - Upload a compact logo (with a global compact logo not having been uploaded before)
    ...
    And I upload "theme/learnr/tests/fixtures/flavourlogo.png" file to "Compact logo" filemanager
    ...
    # We can't check the uploaded image file visually, but we can verify that the compact logo is shipped from the theme_learnr flavour filearea.
    Then "//nav[contains(@class, 'navbar')]//img[contains(@class, 'logo')][contains(@src, 'pluginfile.php/1/theme_learnr/flavours_look_logocompact')][contains(@src, 'flavourlogo.png')]" "xpath_element" should exist
  ...
  Scenario: Flavours: Compact logo - Do not upload a compact logo (with a global compact logo being served properly)
    ...
    And I should see "Flavours" in the "#region-main h2" "css_element"
    And I click on "Preview" "button" in the "#region-main table" "css_element"
    # We can't check the uploaded image file visually, but we can verify that the compact logo is shipped from the theme_learnr global logo filearea.
    Then "//nav[contains(@class, 'navbar')]//img[contains(@class, 'logo')][contains(@src, 'pluginfile.php/1/theme_learnr/logocompact')][contains(@src, 'moodlelogo.png')]" "xpath_element" should exist
  ...
  Scenario: Flavours: Favicon - Upload a favicon (with a global favicon not having been uploaded before)
    ...
    And I should see "Create flavour" in the "#page-header h1" "css_element"
    And I set the field "Title" to "My shiny new flavour"
    And I upload "theme/learnr/tests/fixtures/flavourfavicon.ico" file to "Favicon" filemanager
    And I click on "Save changes" "button"
    And I should see "Flavours" in the "#region-main h2" "css_element"
    And I click on "Preview" "button" in the "#region-main table" "css_element"
    # We can't check the uploaded image file visually, but we can verify that the favicon is shipped from the theme_learnr flavour filearea.
    Then "//head//link[contains(@rel, 'shortcut')][contains(@href, 'pluginfile.php/1/theme_learnr/flavours_look_favicon')][contains(@href, 'flavourfavicon.ico')]" "xpath_element" should exist
  ...
  Scenario: Flavours: Favicon - Do not upload a favicon (with a global favicon being served properly)
    ...
    And I should see "Flavours" in the "#region-main h2" "css_element"
    And I click on "Preview" "button" in the "#region-main table" "css_element"
    # We can't check the uploaded image file visually, but we can verify that the favicon is shipped from the theme_learnr global favicon filearea.
    Then "//head//link[contains(@rel, 'shortcut')][contains(@href, 'pluginfile.php/1/theme_learnr/favicon')][contains(@href, 'favicon.ico')]" "xpath_element" should exist
