...
$PAGE->set_url(new moodle_url('/theme/learnr/smartmenus/edit.php', ['id' => $id, 'sesskey' => sesskey()]));
...
if ($id !== null && $id > 0) {
    $PAGE->set_heading(get_string('smartmenusmenuedit', 'theme_learnr'));
    $PAGE->navbar->add(get_string('smartmenusmenuedit', 'theme_learnr'));
} else {
    $PAGE->set_heading(get_string('smartmenusmenucreate', 'theme_learnr'));
    $PAGE->navbar->add(get_string('smartmenusmenucreate', 'theme_learnr'));
}

// If we are editing an existing menu.
if ($id != null) {
    // Get menu from DB.
    $menu = $DB->get_record('theme_learnr_menus', ['id' => $id], '*', MUST_EXIST);

    // Init form and pass the id and menu object to it.
    $form = new \theme_learnr\form\smartmenu_edit_form(null, ['id' => $id, 'menu' => $menu]);

    // Otherwise, if we are creating a new menu.
} else {
    // Init form and pass the id to it.
    $form = new \theme_learnr\form\smartmenu_edit_form(null, ['id' => $id]);
}

// If the form was submitted.
if ($data = $form->get_data()) {
    // Handle form results.
    $menuid = theme_learnr\smartmenu::manage_instance($data);
    ...
    if (isset($data->saveanddisplay) && $data->saveanddisplay) {
        redirect(new moodle_url('/theme/learnr/smartmenus/items.php', ['menu' => $menuid]));

        // Otherwise.
    } else {
        // Redirect to menu list.
        redirect(new moodle_url('/theme/learnr/smartmenus/menus.php'));
    }
...
if ($id !== null && $id > 0) {
    ...
    if ($record = theme_learnr\smartmenu::get_menu($id)) {
        ...
        \core\notification::error(get_string('error:smartmenusmenunotfound', 'theme_learnr'));
