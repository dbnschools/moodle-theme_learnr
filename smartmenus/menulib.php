...
class smartmenu_helpers {
    ...
    public static function find_condition_used_menus($id, $method='cohorts') {
        ...
        $sql = "SELECT * FROM {theme_learnr_menus} WHERE $like";
        $params = ['value' => '%"'.$id.'"%'];
    ...
    public static function find_condition_used_menuitems($id, $method='cohorts') {
        ...
        $like = $DB->sql_like($method, ':value');
        $sql = "SELECT * FROM {theme_learnr_menuitems} WHERE $like";
        $params = ['value' => '%"'.$id.'"%'];
            ...
            foreach ($menus as $menu) {
                if (isset($menu->$method)) {
                    ...
                    if (($key = array_search($id, $value)) !== false) {
                        ...
                        $DB->set_field('theme_learnr_menus', $method, $updated, ['id' => $menu->id]);
            ...
            foreach ($menuitems as $item) {
                if (isset($item->$method)) {
                    ...
                    if (($key = array_search($id, $value)) !== false) {
                        unset($value[$key]);
                        $updated = json_encode($value);
                        $DB->set_field('theme_learnr_menuitems', $method, $updated, ['id' => $item->id]);
    ...
    public static function purge_cache_dynamic_courseitems() {
        ...
        if ($items = $DB->get_records('theme_learnr_menuitems', ['type' => smartmenu_item::TYPEDYNAMIC])) {
    ...
    public static function set_user_purgecache($userid) {
        ...
        set_user_preference('theme_learnr_menu_purgesessioncache', true, $userid);
    }
    ...
    public static function clear_user_cachepreferencemenu() {
        global $USER;
        set_user_preference('theme_learnr_menu_purgesessioncache', false, $USER);
    }
