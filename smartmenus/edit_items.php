...
$PAGE->set_url(new moodle_url('/theme/learnr/smartmenus/edit_items.php', ['menu' => $menu->id, 'sesskey' => sesskey()]));
...
if ($menuid == null && $id !== null) {
    $PAGE->set_heading(get_string('smartmenusmenuitemedit', 'theme_learnr'));
    $PAGE->navbar->add(get_string('smartmenusmenuitemedit', 'theme_learnr'));
} else {
    $PAGE->set_heading(get_string('smartmenusmenuitemcreate', 'theme_learnr'));
    $PAGE->navbar->add(get_string('smartmenusmenuitemcreate', 'theme_learnr'));
}

// Prepare draft item id for the form.
$draftitemid = file_get_submitted_draft_itemid('image');
file_prepare_draft_area($draftitemid, $context->id, 'theme_learnr', 'smartmenus_itemimage', $id,
        theme_learnr\smartmenu_item::image_filepickeroptions()
);

// Prepare the next menu item id based on the last item ID (if no menu items exist yet, the last item is 0).
$lastmenu = theme_learnr\smartmenu_item::get_lastitem($menu->id);
$nextorder = isset($lastmenu->sortorder) ? $lastmenu->sortorder + 1 : 1;

// Prepare form URL.
$formurl = new moodle_url('/theme/learnr/smartmenus/edit_items.php', ['menu' => $menu->id, 'sesskey' => sesskey()]);

// If we are editing an existing menu item.
if ($id != null) {
    // Get menu item from DB.
    $menuitem = $DB->get_record('theme_learnr_menuitems', ['id' => $id], '*', MUST_EXIST);

    // Init form and pass the id and menu item object to it.
    $form = new \theme_learnr\form\smartmenu_item_edit_form($formurl->out(false), [
            'id' => $id,
            'menu' => $menu->id,
            'menutype' => $menu->type,
            'nextorder' => $nextorder,
...
if ($data = $form->get_data()) {
    // Handle form results.
    $result = theme_learnr\smartmenu_item::manage_instance($data);
    if ($result) {
        // Redirect to menu item list.
        redirect(new moodle_url('/theme/learnr/smartmenus/items.php', ['menu' => $menu->id]));
    }
...
if ($id !== null && $id > 0) {
    ...
    if ($record = theme_learnr\smartmenu_item::instance($id)->get_item()) {
        ...
        \core\notification::error(get_string('error:smartmenusmenuitemnotfound', 'theme_learnr'));
