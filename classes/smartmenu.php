...
class smartmenu {
    ...
    public static function instance($menu) {
        ...
        if (!is_array($menu) && !is_object($menu)) {
            throw new moodle_exception('menuformatnotcorrect', 'theme_learnr');
        }
    ...
    public function __construct($menu) {
        ...
        $this->helper = new \smartmenu_helpers($this->menu);
        // Cache for menu.
        $this->cache = cache::make('theme_learnr', 'smartmenus');
    }
    ...
    public function delete_menu() {
        global $DB;
        if ($DB->delete_records('theme_learnr_menus', ['id' => $this->id])) {
            // Delete all its items.
            $DB->delete_records('theme_learnr_menuitems', ['menu' => $this->id]);
    ...
    public function move_upward() {
        ...
        if ($currentposition > 1) {
            $menus = $DB->get_records('theme_learnr_menus', ['sortorder' => $currentposition - 1]);
            if (empty($menus)) {
                return false;
            }
            $prevmenu = current($menus);
            // Update the menu position to upwards.
            $DB->set_field('theme_learnr_menus', 'sortorder', $prevmenu->sortorder, ['id' => $this->id]);
            // Set the prevmenu position to down.
            $DB->set_field('theme_learnr_menus', 'sortorder', $currentposition, ['id' => $prevmenu->id]);
    ...
    public function move_downward() {
        ...
        if ($currentposition < self::get_menuscount()) {
            ...
            $DB->set_field('theme_learnr_menus', 'sortorder', $nextmenu->sortorder, ['id' => $this->id]);
            // Set the nextmenu position to up.
            $DB->set_field('theme_learnr_menus', 'sortorder', $currentposition, ['id' => $nextmenu->id]);
            // Purge the menus list, need to recreate in new order.
            $this->cache->delete(self::CACHE_MENUSLIST);
    ...
    public function duplicate() {
        ...
        if (!empty($items)) {
            ...
            foreach ($items as $item) {
                $item = \theme_learnr\smartmenu_item::instance($item->id)->item;
                $item->id = 0;
                // New menu id as menu for duplicate item.
                $item->menu = $duplicateid;
                // Create the item.
                \theme_learnr\smartmenu_item::manage_instance($item);
            }
        }
        // Success message for duplicated menu.
        \core\notification::success(get_string('smartmenusmenuduplicatesuccess', 'theme_learnr'));
    ...
    public function update_field($key, $value) {
        ...
        $result = $DB->set_field('theme_learnr_menus', $key, $value, ['id' => $this->id]);
    ...
    public function get_menu_items() {
        ...
        $sql = "SELECT mi.* FROM {theme_learnr_menuitems} mi
            LEFT JOIN {theme_learnr_menus} mn ON mn.id = mi.menu
            WHERE mi.menu=:id
    ...
    public function build($resetcache=false) {
        ...
        if (empty($itemcache)) {
            $itemcache = cache::make('theme_learnr', 'smartmenu_items');
        }
        ...
        smartmenu_helpers::purge_cache_date_reached($this->cache, $this->menu, 'menulastcheckdate');
        ...
        if (!empty($menuitems)) {
            ...
            foreach ($menuitems as $item) {
                ...
                $item = \theme_learnr\smartmenu_item::instance($item, $this->menu)->build();
    ...
    public static function get_menus_forlocation($location, $menus) {
        ...
        $topmenus = smartmenu_helpers::get_menu_cache()->get(self::CACHE_MENUSLIST);
    ...
    public static function get_menu($id) {
        ...
        if ($record = $DB->get_record('theme_learnr_menus', ['id' => $id])) {
    ...
    public static function get_menuscount() {
        global $DB;
        return $DB->count_records('theme_learnr_menus', []);
    }
    ...
    public static function get_lastmenu() {
        global $DB;
        $records = $DB->get_records_sql('SELECT * FROM {theme_learnr_menus} ORDER BY sortorder DESC', [], 0, 1);
    ...
    public static function get_locations() {
        ...
        $locations = [
            self::LOCATION_MAIN => get_string('smartmenusmenulocationmain', 'theme_learnr'),
            self::LOCATION_MENU => get_string('smartmenusmenulocationmenu', 'theme_learnr'),
            self::LOCATION_USER => get_string('smartmenusmenulocationuser', 'theme_learnr'),
            self::LOCATION_BOTTOM => get_string('smartmenusmenulocationbottom', 'theme_learnr'),
        ];
    ...
    public static function get_types() {
        $types = [
            self::TYPE_LIST => get_string('smartmenusmenutypelist', 'theme_learnr'),
            self::TYPE_CARD => get_string('smartmenusmenutypecard', 'theme_learnr'),
        ];
    ...
    public static function manage_instance($formdata) {
        ...
        $cache = cache::make('theme_learnr', 'smartmenus');

        $transaction = $DB->start_delegated_transaction();

        if (isset($formdata->id) && $DB->record_exists('theme_learnr_menus', ['id' => $formdata->id])) {
            $menuid = $formdata->id;

            $DB->update_record('theme_learnr_menus', $record);
    ...
    public static function get_menus() {
        ...
        $topmenus = $DB->get_records('theme_learnr_menus', [], 'sortorder ASC');
        return $topmenus;
    }
    ...
    public static function build_smartmenu() {
        ...
        $removecache = (get_user_preferences('theme_learnr_menu_purgesessioncache', false) == true);
        ...
        \smartmenu_helpers::clear_user_cachepreferencemenu();
    ...
    protected static function verify_lang_session_changes() {
        ...
        if ($lang = optional_param('lang', '', PARAM_SAFEDIR)) {
            ...
            if (!isset($SESSION->prevlang) || $SESSION->prevlang != $lang) {
                ...
                \smartmenu_helpers::set_user_purgecache($USER->id);
