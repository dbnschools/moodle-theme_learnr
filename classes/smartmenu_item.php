...
class smartmenu_item {
    ...
    /**
     * Menu item constructor, Retrive the item data, Create smartmenu_helpers for this item,
    ...
    public function __construct($item, $menu=null) {
        ...
        $this->helper = new smartmenu_helpers($this->item);

        // Cache instance for the items.
        $this->cache = cache::make('theme_learnr', 'smartmenu_items');

        // Menus cache instance.
        // Purge the menu related to the item, when the item is updated, created and sorted.
        $this->menucache = cache::make('theme_learnr', 'smartmenus');
    }
    ...
    public function get_item($itemid=null) {
        ...
        if ($record = $DB->get_record('theme_learnr_menuitems', ['id' => $itemid ?: $this->id ])) {
    ...
    public function delete_menuitem() {
        ...
        if ($DB->delete_records('theme_learnr_menuitems', ['id' => $this->id])) {
            // Reorder the items.
            $this->reorder_items();
            // Delete the cache.
            $this->delete_cache();
    ...
    public function move_upward() {
        ...
        if ($currentposition > 1) {
            ...
            $sql = 'SELECT * FROM {theme_learnr_menuitems} WHERE sortorder < :pos AND menu = :menu ORDER BY sortorder ASC';
            ...
            $DB->set_field('theme_learnr_menuitems', 'sortorder', $previtem->sortorder, [
                'id' => $this->id,
                'menu' => $this->item->menu,
            ]);
            // Set the prevmenu position to down.
            $DB->set_field('theme_learnr_menuitems', 'sortorder', $currentposition, [
                'id' => $previtem->id,
                'menu' => $this->item->menu,
            ]);
    ...
    public function move_downward() {
        ...
        $DB->set_field('theme_learnr_menuitems', 'sortorder', $nextitem->sortorder, [
            'id' => $this->id,
            'menu' => $this->item->menu,
        ]);
    ...
    public function reorder_items() {
        ...
        $sql = 'SELECT * FROM {theme_learnr_menuitems} WHERE menu=:menu ORDER BY sortorder ASC';
        ...
        if (end($records)->sortorder != count($records)) {
            ...
            foreach ($records as $itemid => $item) {
                $DB->set_field('theme_learnr_menuitems', 'sortorder', $i, ['id' => $item->id]);
                $i++;
            }
        }
    }
    ...
    public static function get_lastitem($menuid) {
        ...
        $sql = 'SELECT * FROM {theme_learnr_menuitems} WHERE menu=:menu ORDER BY sortorder DESC';
    ...
    public function duplicate() {
        ...
        if (self::manage_instance($record)) {
            \core\notification::success(get_string('smartmenusmenuitemduplicatesuccess', 'theme_learnr'));
        }
    }
    ...
    public function update_field($key, $value) {
        ...
        return $DB->set_field('theme_learnr_menuitems', $key, $value, ['id' => $this->id]);
    }
    ...
    public function get_itemimage($itemid) {
        ...
        $files = $fs->get_area_files($contextid, 'theme_learnr', 'smartmenus_itemimage', $itemid, '', false);
        if (!empty($files)) {
            // Get the first file.
            $file = reset($files);
    ...
    public function build() {
        ...
        smartmenu_helpers::purge_cache_date_reached($this->cache, $this->item, 'itemlastcheckdate');
    ...
    public function convert_background_code() {
        ...
        $background = smartmenu_helpers::color_get_rgba($this->item->backgroundcolor, self::BACKGROUND_OPACITY);
    ...
    public static function get_itemscount($menuid) {
        global $DB;
        return $DB->count_records('theme_learnr_menuitems', ['menu' => $menuid]);
    }
    ...
    public static function get_types(int $type=null) {
        $types = [
                self::TYPESTATIC => get_string('smartmenusmenuitemtypestatic', 'theme_learnr'),
                self::TYPEHEADING => get_string('smartmenusmenuitemtypeheading', 'theme_learnr'),
                self::TYPEDYNAMIC => get_string('smartmenusmenuitemtypedynamiccourses', 'theme_learnr'),
        ];
    ...
    public static function get_display_options(int $option=null) {
        $displayoptions = [
                self::DISPLAY_SHOWTITLEICON => get_string('smartmenusmenuitemdisplayoptionsshowtitleicon', 'theme_learnr'),
                self::DISPLAY_HIDETITLE => get_string('smartmenushidetitle', 'theme_learnr'),
                self::DISPLAY_HIDETITLEMOBILE => get_string('smartmenushidetitlemobile', 'theme_learnr'),
        ];
    ...
    public static function manage_instance($formdata) {
        ...
        // Cache for menus.
        $menucache = cache::make('theme_learnr', 'smartmenus');
        // Cache for menu items.
        $cache = cache::make('theme_learnr', 'smartmenu_items');

        if (isset($formdata->id) && $oldrecord = $DB->get_record('theme_learnr_menuitems', ['id' => $formdata->id])) {
            $itemid = $formdata->id;

            $DB->update_record('theme_learnr_menuitems', $record);
            ...
            if ($oldrecord->sortorder != $record->sortorder) {
                ...
                $decrease = "UPDATE {theme_learnr_menuitems} $operation AND id != :item AND menu=:menuid";
            ...
            \core\notification::success(get_string('smartmenusmenuitemeditsuccess', 'theme_learnr'));
        } else {
            $record->sortorder = $record->sortorder ?: 1;
            $itemid = $DB->insert_record('theme_learnr_menuitems', $record);
            // Setup the order for item.
            $sql = "UPDATE {theme_learnr_menuitems}
            ...
            \core\notification::success(get_string('smartmenusmenuitemcreatesuccess', 'theme_learnr'));
